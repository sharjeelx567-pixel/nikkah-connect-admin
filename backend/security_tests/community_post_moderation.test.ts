// Emulator-only regression tests for Community Discussion admin moderation
// (community-posts.controller.ts, and the post_reports extension to
// reports.controller.ts). Split out of post_moderation.test.ts when
// Community Discussion was separated from Rishta Posting — this suite
// covers community_posts + their comments subcollection (Rishta Posting
// has no comments; see rishta_post_moderation.test.ts). Run via firebase
...
async function main() {
  ...
  //    target community post + comment + post_report to moderate ────────
  const viewOnlyRef = db.collection('admins').doc(`cpview_${stamp}`);
  await viewOnlyRef.set({ email: `cpview_${stamp}@test.local`, role: 'analyst', permissions: ['posts.view'], isActive: true });
  const viewOnlyToken = signAccessToken({ uid: `cpview_${stamp}`, email: 'x', role: 'analyst' as any });

  const moderatorRef = db.collection('admins').doc(`cpmod_${stamp}`);
  await moderatorRef.set({ email: `cpmod_${stamp}@test.local`, role: 'moderator', isActive: true });
  const moderatorToken = signAccessToken({ uid: `cpmod_${stamp}`, email: 'x', role: 'moderator' as any });

  const authorUid = `cp_author_${stamp}`;
  await db.collection('users').doc(authorUid).set({ displayName: 'Post Author', email: 'author@test.local' });
  const reporterUid = `cp_reporter_${stamp}`;
  await db.collection('users').doc(reporterUid).set({ displayName: 'Reporter', email: 'reporter@test.local' });

  const postRef = db.collection('community_posts').doc(`post_${stamp}`);
  await postRef.set({
    id: postRef.id, communityId: 'test_comm', authorUid, authorDisplayName: 'Author',
    text: 'Meri family shaadi ke liye agree nahi kar rahi.',
  ...
  await commentRef.set({
    ...
    text: 'inappropriate comment', status: 'active', parentCommentId: null, createdAt: new Date(), updatedAt: new Date(),
  });

  const postReportRef = db.collection('post_reports').doc(`${postRef.id}_${reporterUid}`);
  await postReportRef.set({
    id: postReportRef.id, reporterId: reporterUid, contentType: 'community_post', postId: postRef.id,
    targetAuthorUid: authorUid, reason: 'Suspicious profile', status: 'pending', createdAt: new Date(),
  });

  console.log('\n=== TEST 1: GET /community-posts requires posts.view (403 without it) ===');
  {
    const noPermRef = db.collection('admins').doc(`cpnoperm_${stamp}`);
    await noPermRef.set({ email: `cpnoperm_${stamp}@test.local`, role: 'support_staff', isActive: true });
    const noPermToken = signAccessToken({ uid: `cpnoperm_${stamp}`, email: 'x', role: 'support_staff' as any });

    const res = await fetch(`${base}/community-posts`, { headers: { Authorization: `Bearer ${noPermToken}` } });
    assert(res.status === 403, `a role without posts.view gets 403 (got ${res.status})`);

    const okRes = await fetch(`${base}/community-posts`, { headers: { Authorization: `Bearer ${viewOnlyToken}` } });
    assert(okRes.status === 200, `a role WITH posts.view gets 200 (got ${okRes.status})`);
  }

  console.log('\n=== TEST 2: PATCH /community-posts/:id/hide requires posts.manage (view-only is rejected) ===');
  {
    const res = await fetch(`${base}/community-posts/${postRef.id}/hide`, {
      method: 'PATCH',
  ...
  console.log('\n=== TEST 3: a posts.manage admin CAN hide a post — writes status/moderatedBy/moderatedAt and an audit log ===');
  {
    ...
    assert(doc.data()?.moderatedBy === `cpmod_${stamp}`, 'moderatedBy is set to the acting admin uid');
  ...
  console.log('\n=== TEST 4: a posts.manage admin can hide a comment (incl. reply parentCommentId passthrough) — writes status and an audit log with targetType comment ===');
  {
    const res = await fetch(`${base}/community-posts/${postRef.id}/comments/${commentRef.id}/hide`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${moderatorToken}`, 'Content-Type': 'application/json' },
  ...
  console.log('\n=== TEST 5: GET /community-posts/comments returns the comment with its parentCommentId field ===');
  {
    const res = await fetch(`${base}/community-posts/comments?postId=${postRef.id}`, { headers: { Authorization: `Bearer ${moderatorToken}` } });
    assert(res.status === 200, `GET /community-posts/comments succeeds (got ${res.status})`);
    const body: any = await res.json();
    const items: any[] = body.data?.data || [];
    const found = items.find((it) => it.id === commentRef.id);
    assert(!!found, 'the seeded comment appears in the listing');
    assert(found?.parentCommentId === null, 'a top-level comment reports parentCommentId: null');
  }

  console.log('\n=== TEST 6: GET /reports returns the merged post_reports item tagged contentType=community_post ===');
  {
    ...
    assert(found?.contentType === 'community_post', 'the item carries contentType=community_post');
    assert(found?.category === 'Reported Community Post', `category label is "Reported Community Post" (got ${found?.category})`);
  }

  console.log('\n=== TEST 7: PATCH /reports/:id/resolve on the post_reports-sourced id writes status AND an audit log (the pre-existing gap this closes) ===');
  {
    ...
    assert(doc.data()?.resolvedBy === `cpmod_${stamp}`, 'resolvedBy is set to the acting admin uid');
  ...
  {
    const res = await fetch(`${base}/community-posts/${postRef.id}/unhide`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${moderatorToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    });
